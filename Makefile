MODULE  := github.com/nlink-jp/json-filter
BINARY  := json-filter
BIN_DIR := dist
...
## build: Build binary for the current OS/Arch → ./dist/json-filter
build:
	@mkdir -p $(BIN_DIR)
...
## package: Cross-compile, sign, archive (zip for darwin/windows, tar.gz
## for linux), bundle the canonical binary + README.md + LICENSE, and
## notarize the darwin build → dist/. Asset naming follows the org
## Release Archive Standard (json-filter-vX.Y.Z-<os>-<arch>.<ext>).
package: build-all
...
BREW_KIND := formula
BREW_DESC := Robust JSON extraction, validation, and repair filter
